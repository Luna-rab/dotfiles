...
代わりの検査をスクリプト側に置いた。`runReview` が `missing`（起動できなかったレビューの数）を
返し、1 件でもあればそのラウンドで打ち切る。**ワークフローのスクリプトは信用してよい**という
前提を置いている。リードは返り値の `reviewers` が `tier` と合っているかを目で見るだけにする。
...
- **`TaskList` の `completed` を完了の根拠にしない。** subagent が終了すると harness が spawn 元の
  ...
  `review.py list --require-empty` の 3 つだけである。
