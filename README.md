...
左ペインは「ラン → タスク → ステージ」のリストを 1 層ずつ出す。↑↓ で選ぶと右ペインに詳細が出て、
Enter（→）で 1 つ深い層に入り、Esc（← / Backspace）で戻る。`q` で終わる。

| 選んでいるもの | 右ペインに出すもの |
| --- | --- |
| ラン | ゴール（起動時の指示とまとめステージの説明）・進み具合（スタック済みの数とタスクの一覧）・判断ログ・スコープ外 |
| タスク | ステージの並び・ステージの履歴（成否・ラウンド・ターン数）・指摘の件数と未解決の指摘・受入条件・DoD・範囲 |
| ステージ | ステージに渡した指示と、それに続けて Claude Code の出力（発言は全文、ツールは呼び出しを 1 行。結果は出さない）。開くと指示の先頭を出し、末尾までスクロールしているときだけ新しい出力を追う |

- タスクのリストの先頭の「準備と仕上げ」から、どのタスクにも属さない計画ステージとまとめステージに入れる。
- 起動するとランのリストから始まる（ラン名を渡すとそのランのタスクから）。2 秒ごとに読み直すが、
  選んでいる項目は保ち、新しいステージが始まってもカーソルを動かさない。何も書き込まない。
- **ステージに渡した指示は、driver が起動のたびに `logs/<タスク>/<ステージ>-<ラウンド>.prompt.md` に
  書き残す。** この仕組みより前に走ったステージには記録が無く、「指示の記録が無い」と出る。
