...
| 3 | 要対応のタスクがある（要確認 `blocked` か失敗 `failed`。この仕組みより前に始めたランだけ） |
| 4 | 人の回答を待って止まっている（下） |
...
## 進めなくなったら、driver が手を変える

**指摘はすべてそのタスクの中で解決する。** ジャッジが却下してよいのは、根拠を示せる誤った指摘と
nit だけである。**ラウンドに上限は無く**、未解決が 0 件になるまで回す。

止まらずに回り続けるのを防ぐのは**停滞の検知**である。次のどちらかで、ジャッジが原因を分類する。

- 同じ指摘が、修正を 2 回受けても未解決のまま（driver が数える。ラウンドごとに新しく立つ指摘は数えない）
- 経緯から、このタスクの中では直せないとジャッジが判断した

ジャッジは同じタスクの間、同じセッションで呼ばれるので、前のラウンドの経緯を覚えている。

| 原因 | driver が打つ手 |
| --- | --- |
| テストが誤り | テスト作成ステージが受入条件と照らして直し、修正を続ける |
| 範囲の中で堂々巡り | 実装を新しいセッションでやり直す |
| 範囲の外が要る（割り方の問題） | 再計画ステージがタスクの割り方を直す |
| 受入条件が曖昧 | 人に聞いて回答を待つ |

再計画ステージは、止まったタスクの範囲を広げる・指摘を同じランの後ろのタスクへ移す・未着手の
タスクを組み直す・止まったタスクを取り下げて割り直す、のどれかを選ぶ。**スタック済みのタスクには
触らない。** 移した指摘は移した先の受入条件に足され、そのタスクのジャッジが閉じるまで
スタックに追加されない。**再計画はラン全体で 2 回まで**で、超えたら人に聞く。これが最後の歯止めである。

ほかの止まり方も、回答待ちか修正のループに寄せてある。

- 完了チェックが落ちたら、落ちた項目を must-fix の指摘にして修正のループへ戻す
- ステージがエラーで終わったら 1 回だけ呼び直し、それでも落ちたら人に聞く
- テスト作成・実装ステージが「受入条件が曖昧」と返したら、人に聞く
...
flowchart TD
    ...
    subgraph driver["driver（決定的。状態遷移・停滞の検知・再計画の回数を持つ）"]
        ...
        jd["ジャッジ（タスクの間は同じセッション）"] --> decide{"未解決の指摘が 0？"}
        decide -->|いいえ| route{"停滞の原因"}
        route -->|なし| fix["修正ステージ"] --> rv
        route -->|テスト| tg2["テスト作成ステージが直す"] --> fix
        route -->|堂々巡り| redo["実装を新しいセッションでやり直す"] --> rv
        route -->|割り方| replan["再計画ステージ（ラン全体で 2 回まで）"] --> loop
        route -->|曖昧| ask["人に聞いて回答待ち"]
...
6. **レビューは毎ラウンドまっさらにする。** 1 ラウンド目の結論を持ち込むと、それがフレーミングに
   なって検出が落ちる。セッションを続けるのは実装・修正（同じセッション）とジャッジだけで、
   ジャッジは経緯を覚えていることが停滞の見分けに要る。
...
| | 効くもの |
| --- | --- |
| モデルと思考量 | 読んで決めるステージは `opus`、文章を組むだけのステージ（PR 本文・まとめ）は `sonnet`。思考量は計画・再計画・ジャッジが `high`、ほかは `medium` |
