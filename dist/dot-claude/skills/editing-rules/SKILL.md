...
paths:
  - ".claude/rules/**"
  - "**/.claude/rules/**"
  # dotfiles リポジトリは配布物を dist/ に置く。ここが ~/.claude/rules へ配られる実体
  - "dist/dot-claude/rules/**"
  - ".claude/CLAUDE.md"
  - "**/CLAUDE.md"
  - "**/AGENTS.md"
allowed-tools:
...
- **ルールに置くのは「常に守る規則」だけ。** 手順や作業のやり方はスキルにする。
- 全プロジェクトに効かせるルールは `dist/dot-claude/rules/` に置く。`install.sh` が
  `~/.claude/rules` へ symlink する（ディレクトリごとなので、ファイルを足せばすぐ効く）。
- このリポジトリでだけ効かせたいルールは `.claude/rules/` に置く。ここは配らない。
