...
paths:
  ...
  - ".claude/commands/**"
  - "**/.claude/commands/**"
  # dotfiles リポジトリは配布物を dist/ に置く。ここが ~/.claude/skills へ配られる実体
  - "dist/dot-claude/skills/**"
  - "dist/dot-claude/commands/**"
allowed-tools:
...
| **相対リンクと `scripts/` 参照が実在するか** | dotfiles リポジトリで作業しているなら `.claude/scripts/check-skills.py` を流す（配布しないので、ほかのリポジトリには無い）。frontmatter の YAML・行数・リンク・実行権限を検査する |
